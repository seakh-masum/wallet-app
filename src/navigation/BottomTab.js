  ...
  const setTabConfig = (route, focused) => {
    if (route.name === 'Cards') {
      return (
        <View
          className={`${focused ?
            'flex-row gap-1 items-center bg-pink-100 rounded-3xl pl-3 pr-4 pt-1 pb-2' : 'mb-1'
            }`}>
          <CreditCardIcon
      ...
      return (
        <View
          className={`${focused ?
            'flex-row gap-1 items-center bg-indigo-100 rounded-3xl pl-3 pr-4 pt-1 pb-2' : 'mb-1'
            }`}>
          <DocIcon
            fill={focused ? Colors.violet : '#000'}
            isFocus={focused}
            height={focused ? 24 : 32}
            width={focused ? 24 : 32}
          />
